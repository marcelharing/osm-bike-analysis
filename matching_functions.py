...
PAPER:      Vierø A. R., Vybornova A., Szell M. (2023). 
            BikeDNA: A tool for bicycle infrastructure data and network assessment.
            Environment and Planning B: Urban Analytics and City Science. https://doi.org/10.1177/23998083231184471.

This script contains files used for the feature matching of two different datasets of the same road network.
All credits go to the original authors of the functions.
""" 
...
def _get_angle(linestring1, linestring2):
    ...
    angle = math.atan2(np.linalg.det([arr1, arr2]), np.dot(arr1, arr2))
...
def find_matches_from_buffer(
    ...
    print(f"{len(matched_data)} reference segments were matched")

    print(
        f"{ len(reference_data) - len(matched_data) } segments were not matched"
    )
